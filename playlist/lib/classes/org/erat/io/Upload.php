<?php
// $Id: Upload.php,v 1.2 2003/12/23 01:31:23 admin Exp $
class Upload {
	// class variable
	...
	// input: $overwrite = allow FILE to overwrite a file already existing?
	function putFile ( $file, $overwrite="no" ) {
        ...
        if ( $error == "" ) {
			...
			if ( (file_exists($destination)) && ($overwrite=="no") ) {
	...
	// 			$post_name   = post file string for thumbs				default -> "t"
	function miniatura ( $image_path, $path, $larghezza=80, $altezza=80, $pre_name="thumb_", $post_name="t" ) {
		if ( (eregi("\.png$", $image_path) || eregi("\.(jpg|jpeg)$", $image_path)) && $image_path ) {
			$image_name = basename ( $image_path );
			$newName = $pre_name.substr($image_name,0,strlen($image_name)-4).$post_name.substr($image_name,strlen($image_name)-4);
			if (!empty ($path) ) 
				$thumb_path = $path."/".$newName; // complete path to thumbnail dir
			else 
				$thumb_path = $pre_name.$newName; // complete path to thumbnail dir
	...
	function getDimension ( $image_file_path, $dim="both" ) {
	    	// create new image
	    	if(eregi("\.png$",$image_file_path)) {
				$return_val = ( ($img = ImageCreateFromPNG ( $image_file_path )) && $return_val == 1 ) ? "1" : "0";
	    	}
	    	...
	    	if ($dim=="width") return imagesx ($img);    // original width
	    	else if ($dim=="height") return imagesy ($img);    // original width
	    	else return imagesx($img)."x".imagesy($img);
	}
